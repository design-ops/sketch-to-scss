 ...
 */
async function copyFonts(names, embeddedFontData, outputFolder, base64) {

    let folder

    if (!base64) {
      // Make sure the output folder exists
      folder = outputFolder + "/assets/fonts/"
    ...
    for (let nameIndex = 0; nameIndex < names.length; ++nameIndex) {
        ...
        ret.push(await performCopy(name, embeddedFontData, allFonts, folder, base64))
    }
    return ret
}

async function performCopy(name, embeddedFontData, allFonts, folder, base64) {

    // First we try to copy the files from the embedded font references.
    // If the font name is not found in the embedded fonts, we try to export from the system fonts
    const extractDirectory = path.join(os.tmpdir(), "sketch-to-swiftui")
    await decompress(embeddedFontData.sketchFile, extractDirectory)
    for (let fontReferenceIndex = 0; fontReferenceIndex < embeddedFontData.usedFontReferences.length; fontReferenceIndex++) {
        const fontRef = embeddedFontData.usedFontReferences[fontReferenceIndex];
        if(fontRef.postscriptNames.includes(name) && fontRef.fontData) {
            let fontPath;
            if (base64) {
                // Do base64 encoding
                let buf = fs.readFileSync(path.join(extractDirectory, fontRef.fontData._ref))
                fontPath = `data:font/${fontFormat(fontRef.fontFileName)};base64,${buf.toString('base64')}`
                // path = "base64encoded"
            } else {
                // Copy the font reference to the output folder
                const origin = path.join(extractDirectory, fontRef.fontData._ref)
                const destination = path.join(folder, fontRef.fontFileName)
                fs.copyFileSync(origin, destination)
                fontPath = encodeURI("./assets/fonts/" + fontRef.fontFileName)
            }

            return {
                family: name,
                path: fontPath,
                format: fontFormat(fontRef.fontFileName)
            }
        }
    }
    ...
    if (base64) {
        let buf = fs.readFileSync(font.fullPath)
        return {
            family: name,
            path: `data:font/${fontFormat(fontFileName)};base64,${buf.toString('base64')}`,
            format: fontFormat(fontFileName)
        }
    }
    // Create a copy of that font, pointing it at the final location. Add that copied font to the output
    let returnedFont = {
        family: name,
        path: encodeURI("./assets/fonts/" + fontFileName),
        format: fontFormat(fontFileName)
    }
