...
async function processAssets(layers, filepath, outputFolder) {
  ...
  for (let index = 0; index < layers.length; ++index) {
    ...
    let backgroundImage = await assetName(layer, filepath, outputFolder)
    css.push({
 ...
 */
async function assetName(layer, filepath, outputFolder) {
  let options = {
  ...
  if (icon.svg.length > 1024 * 2) {
    ...
    // If the icon is small enough, we can just embed it directly.
